...
def calculate():
    ...
    characterLabel.config(text=f"karacters: {getNumberOfCharacters(textToBeCalculated)}")
    ...
    aviLabel.config(text =f"avi level: {AVI(textToBeCalculated)}")
...
aviLabel = tk.Label(root, text=f'avi level:', width=20, bg='black', fg='white')
...
wordLabel.place(x = 320, y = 560)
aviLabel.place(x = 480, y = 560)
